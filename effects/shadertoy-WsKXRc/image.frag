...
vec3 render(in vec2 fragCoord){
	vec3 color = vec3(0.);
    float a = (iResolution.x - iMouse.x*0) * .05;
    ...
    if(sphere_hit(Sphere(vec3(0.), 1.00001), camRay, MIN_FLOAT, MAX_FLOAT, rec)){
        ...
        for(int i=0; i<MAX_MARCHING_STEPS; i++)	{
            ...
            if(abs(pow(d, .5) - min(pow(sp.y, .75), (.75 - pow(length(sp.xz), 64.)))) <= .05)
            	color += hsv2rgb_2(vec3(fbm1x(d, iTime * .1 + 100.) * 3.1415, 1., 1.)) * .015;
            t += .01;
        }
        ...
        for(int i=0; i<MAX_MARCHING_STEPS; i++)	{
            ...
            d = map(sp);
            if(abs(pow(d, .5) - min(pow(sp.y, .75), (.75 - pow(length(sp.xz), 64.)))) <= .05)
            	fresCol += .05 * hsv2rgb_2(vec3(fbm1x(d, iTime * .1 + 100.) * .5415, 1., 1.));
            t += .0075;
        }
        color += fresCol * dt;
        color += pow(texture(iChannel1, reflect(worldDir, nrm).xy).rgb, vec3(4.)) * .1;
    }
    return color;
}
