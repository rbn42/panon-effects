...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    for(int line=int(fragCoord.x-glow_radius)/gap_line ;
            ...
            line++){
        for(int y=0*line%gap_y +int(fragCoord.y-glow_radius)/gap_y*gap_y ;
                y<fragCoord.y+glow_radius;
                y+=gap_y){
            int x=line*gap_line+y/gap_y*0;
            float l=length(vec2(x,y)-fragCoord);

            float index_x=line*gap_line/iResolution.x;
            float index_y=y/iResolution.y;
            vec3 rgb=getRGB(index_x);

            vec4 sample1= texture(iChannel1, vec2(index_x,0)) ;
            if(sample1.r>index_y){
