...
void dance1()
{
    ...
    stance[ANKLES].x = -s1.g*1.0;
    s1=texelFetch(iChannel2,ivec2(3,0), 0);
    stance[ANKLES].z = -s1.g*1.0;
    stance[ANKLES].xz -= -stance[KNEES].xz*1.0;
    ...
    //腕
    s1=texelFetch(iChannel2,ivec2(1,1), 0);
    stance[WRISTS].x = (stance[ELBOWS].x*.5+.5*s1.g);
    s1=texelFetch(iChannel2,ivec2(1,0), 0);
    stance[WRISTS].z = (stance[ELBOWS].z*.5+.5*s1.g);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    float height = 0; //max(lowAverage(), last_height)-1.0/60.0;
    last_height = height;
    ...
    //rotate camera
    ro.yz *= rot(sin(iTime*0) * 0.25);
    rd.yz *= rot(sin(iTime*0) * 0.25); 
    ro.xz *= rot(iTime*0 * 0.5);
    rd.xz *= rot(iTime*0 * 0.5);
    //*/
    
    eye = ro;
    
    fragColor = vec4(marchScene(ro, rd), 0.0);    
